...
echo \"type\",\"language\",\"filename\",\"graph\",\"reordering\",\"NES\",\"NWES\",\"time\",\"bandwidth\",\"profile\",\"span\",\"avgwavefront\",\"RMSwavefront\",\"reachtime\",\"peaknodes\"
...
do
	...
	# print language
	frontend=$(grep "[^2]*2lts-sym" $i -m1 -o)
	case $frontend in
		"pnml2lts-sym")
		echo -n \"PNML\",
		;;
	...
	grep "prob2lts-sym" $i > /dev/null
	if [ $? = 0 ]
	then
		# example
		# prob2lts-sym --zocket-prefix=/tmp/ltsmin-112213- --no-close spec/B/RushHour_v2_TLC.mch -rsc,bg,vcm --regroup-exit

		if [ $(grep "prob2lts-sym --zocket-prefix=/tmp/ltsmin-" $i | wc -l) = 1 ]
		then			
			line=$(grep "prob2lts-sym --zocket-prefix=/tmp/ltsmin-" $i)			
			echo $line | awk '{
				printf "\"%s\"", $4
			}'
		else
		...
		if [ $(grep "pnml2lts-sym: opening" $i | wc -l) = 1 ]
		then
			awk '{
				if ($1" "$2 == "pnml2lts-sym: opening") printf "\"%s\"", $3
			}' $i		
		fi	
	fi
	echo -n ,
	...
	# print reachability time
	if [ $(grep "reachability took" $i | wc -l) = 1 ]
	then
		awk '{
			if ($2" "$3 == "reachability took") printf "\"%s\"", $6
		}' $i		
	fi
	echo -n ,
	
	# print peak nodes
	if [ $(grep "peak nodes )" $i | wc -l) = 1 ]
	then
		awk '{
			if ($8" "$9" "$10 == "peak nodes )") printf "\"%s\"", $7
		}' $i		
	fi
